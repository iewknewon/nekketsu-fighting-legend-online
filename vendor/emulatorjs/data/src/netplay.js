...
export class Netplay {
    constructor(emu) {
        ...
        this.videoCaptureFps = Math.min(60, Math.max(30, Number(this.emu.config.netplayVideoFps || window.EJS_netplayVideoFps || 60)));
        this.syncLeadFrames = Math.min(20, Math.max(2, Number(this.emu.config.netplaySyncLeadFrames || window.EJS_netplaySyncLeadFrames || 8)));
        ...
        if (this.owner && this.localStream) {
            ...
            try {
                ...
                if (sender) {
                    ...
                    p.encodings[0].maxFramerate = this.videoCaptureFps;
    ...
    simulateInput(player, index, value) {
        ...
        if (this.owner) {
            ...
            const sentViaPeer = this.emu.gameManager.functions.simulateInput(pidx, index, value) === true;
            if (!sentViaPeer) {
                this.sendMessage({ "sync-control": [{ frame: f + this.syncLeadFrames, connected_input: [pidx, index, value] }] });
            }
        }
    }
    ...
    _initModulePostMainLoop() {
        ...
        if (this.inputsData[i]) {
            const ts = this.inputsData[i].map((v) => {
                ...
                return { frame: i + this.syncLeadFrames, connected_input: v.connected_input };
            });
